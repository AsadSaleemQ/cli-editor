...
$builderSource = [IO.File]::ReadAllText((Join-Path $repository 'scripts\build_release_ai.ps1'))
...
$bundle = Join-Path $caseRoot ".artifacts\cli-editor-$version-windows-x64"
...
$savedSeed = $env:CLI_EDITOR_SIGNING_PRIVATE_KEY_HEX
try {
    ...
    [IO.File]::Copy((Resolve-Path $SignerPath), (Join-Path $releaseTools 'sign_release_ai.exe'), $false)
    ...
    foreach ($name in @('cli-editor.exe', 'codex-enhanced.exe', 'codex-code-mode-host.exe')) {
        [IO.File]::WriteAllText((Join-Path $bundle $name), "fixture-$name", (New-Object Text.UTF8Encoding($false)))
    }
    foreach ($name in @('LICENSE', 'NOTICE', 'THIRD_PARTY_NOTICES_ai.md', 'THIRD_PARTY_LICENSES_CLI_EDITOR_ai.html', 'THIRD_PARTY_LICENSES_CODEX_ai.html')) {
        [IO.File]::WriteAllText((Join-Path $bundle $name), "fixture-$name", (New-Object Text.UTF8Encoding($false)))
    }
    ...
    $artifacts = foreach ($name in @('cli-editor.exe', 'codex-enhanced.exe', 'codex-code-mode-host.exe')) {
    ...
    $env:CLI_EDITOR_SIGNING_PRIVATE_KEY_HEX = '9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60'
    ...
    $duplicateManifestPath = Join-Path $duplicateRoot ".artifacts\cli-editor-$version-windows-x64\compatibility-manifest.json"
    ...
    try {
        & (Join-Path $repository 'scripts\finalize_release_ai.ps1') -Version $version -IssuedUnix $issued -ManifestSequence $sequence -ExpiresUnix $expires -RepositoryRoot $duplicateRoot
    } catch {
    ...
    & (Join-Path $repository 'scripts\finalize_release_ai.ps1') -Version $version -IssuedUnix $issued -ManifestSequence $sequence -ExpiresUnix $expires -RepositoryRoot $caseRoot
    & (Join-Path $repository 'scripts\finalize_release_ai.ps1') -Version $version -IssuedUnix $issued -ManifestSequence $sequence -ExpiresUnix $expires -RepositoryRoot $secondRoot
    $firstArchive = Join-Path $caseRoot ".artifacts\cli-editor-$version-windows-x64.zip"
    $secondArchive = Join-Path $secondRoot ".artifacts\cli-editor-$version-windows-x64.zip"
    ...
    foreach ($path in @(
        ...
        (Join-Path $caseRoot ".artifacts\cli-editor-$version-windows-x64.zip"),
        (Join-Path $caseRoot ".artifacts\cli-editor-$version-windows-x64.zip.sha256")
    )) {
    ...
    if ($null -eq $savedSeed) { Remove-Item Env:CLI_EDITOR_SIGNING_PRIVATE_KEY_HEX -ErrorAction SilentlyContinue }
    else { $env:CLI_EDITOR_SIGNING_PRIVATE_KEY_HEX = $savedSeed }
